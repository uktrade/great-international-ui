...
def test_unsubscribe_required_params(client):
    response = client.get(reverse('find-a-supplier:unsubscribe'), {'uidb64': 'aBcD', 'token': '1a2b3c'})
...
def test_unsubscribe_api_validation_failure(mock_unsubscribe, client):
    ...
    response = client.post(reverse('find-a-supplier:unsubscribe'), {'uidb64': 'aBcD', 'token': '1a2b3c'})
...
def test_unsubscribe_api_error(mock_unsubscribe, client):
    ...
    with pytest.raises(requests.exceptions.HTTPError):
        client.post(reverse('find-a-supplier:unsubscribe'), {'uidb64': 'aBcD', 'token': '1a2b3c'})
...
def test_unsubscribe_api_success(mock_unsubscribe, client):
    ...
    mock_unsubscribe.assert_called_once_with('aBcD', '1a2b3c')
    assert response.status_code == 200
